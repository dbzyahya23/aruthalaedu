...
import { ArrowLeft, Medal, Trophy, Star, Flame } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="card card-padding flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="page-subtitle">UI untuk ranking harian, mingguan, bulanan, dan semester.</p>
        </div>
        <Link href="/student-hub" className="btn-outline inline-flex items-center gap-2 self-start lg:self-auto">
          <ArrowLeft className="h-4 w-4" /> Kembali ke Hub Siswa
        </Link>
      </div>
        ...
        {[
          ...
          return (
            <div key={item.label} className="card card-padding">
              <div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${item.tone === "blue" ? "bg-blue-50 text-blue-600" : item.tone === "amber" ? "bg-amber-50 text-amber-600" : item.tone === "red" ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600"}`}>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="card card-padding lg:col-span-2 space-y-4">
          ...
          <div className="space-y-3">
            {TOP_STUDENTS.map((student) => (
              <div key={student.rank} className="flex items-center gap-4 rounded-2xl border border-[#e3ebfa] bg-white/70 p-4">
                <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#eef5ff] text-sm font-bold text-[#2f66e9]">
                  {student.rank}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-gray-900">{student.name}</p>
                  <p className="text-xs text-gray-500">{student.className} · {student.badge}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-gray-900">{student.xp} XP</p>
                  <p className="text-xs text-gray-400">{student.trend}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="card card-padding space-y-4">
          ...
          <div className="rounded-2xl border border-[#d8e6fb] bg-[#f7fbff] p-4 text-sm text-[#2f66e9]">
            Gunakan halaman ini untuk komponen ranking dan leaderboard sekolah.
          </div>
        </div>
      </div>
    </div>
