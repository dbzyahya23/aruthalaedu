  ...
  return (
    <div className="space-y-6">
      <div className="card card-padding flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="page-title">Exam Health</h1>
          <p className="page-subtitle">Pemantauan kesehatan ujian: internet, sync, browser, battery, dan device.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {HEALTHS.map((health) => (
          <div key={health.title} className="card card-padding">
            ...
            <p className={`mt-2 text-3xl font-bold tracking-tight ${health.tone === "green" ? "text-green-600" : "text-amber-600"}`}>{health.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="card card-padding lg:col-span-2 space-y-4">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {[
              ...
              return (
                <div key={item.label} className="flex items-center gap-3 rounded-2xl border border-[#e3ebfa] bg-white/70 p-4">
        ...
        <div className="card card-padding space-y-4">
          ...
          <div className="rounded-2xl border border-[#d8e6fb] bg-[#f7fbff] p-4 text-sm text-[#2f66e9]">
            Ideal untuk dipantau guru selama sesi ujian aktif.
          </div>
        </div>
      </div>
    </div>
