...
const ROUTES = [
  ...
  { href: "/leaderboard", title: "Leaderboard", desc: "Ranking mingguan, hall of fame, dan milestone prestasi.", icon: CalendarRange },
        ...
        <div className="max-w-3xl space-y-3">
          ...
          <div>
            ...
            <p className="page-subtitle">
              Halaman ini jadi pusat navigasi UI untuk modul yang belum selesai, supaya backend team bisa langsung menempelkan data dan action di titik yang tepat.
            </p>
          </div>
        </div>
        ...
        {ROUTES.map((route) => {
          ...
          return (
            <Link
              key={route.href}
              href={route.href}
              className="card card-padding group flex items-start gap-4 hover:border-[#d8e6fb] hover:shadow-[0_22px_50px_rgba(57,111,190,0.12)] transition-all"
            >
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="card card-padding lg:col-span-2 space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {[
              ...
              "School Health: analitik stabilitas dan benchmark",
            ].map((item) => (
        ...
        <div className="card card-padding space-y-4">
          ...
          <Link href="/overview" className="btn-outline inline-flex items-center gap-2">
            <CalendarDays className="h-4 w-4" /> Kembali ke Dashboard
          </Link>
        </div>
      </div>
    </div>
